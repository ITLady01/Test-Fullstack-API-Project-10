...
export default class CourseDetail extends Component {
    ...
    deleteButton() {
        this.setState(prevState => ({
            deleteClicked: !prevState.deleteClicked
        }));
    }
        ...
        return (
...

            {
                deleteClicked ? ( <span>
                    ...
                    onClick = {
                        this.delete
                    } > Yes </button> <button className = "button"
                    onClick = {
                        this.deleteButton
                    } > No </button> </span>
                ) : ( <span > {
                        authUser.id === course.userId ? ( <span >
                            <Link to = {
                                `${courseURL}/update`
                            }
            ...
            className = "button button-secondary" > Return to List </Link> 
            </div> 
            </div>
         </div> 
         <div className = "bounds course--detail" >
            ...
            <h4 className = "course--label" > Course 
            </h4> <h3 className = "course--title" > {
                course.title
            } </h3> <p> by 
            {
                creator.name
